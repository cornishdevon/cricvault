export function Green() {
  const p = {
    bg: "#F5F0E8", card: "#FFFDF8", primary: "#1B5E2B", primaryFg: "#fff",
    fg: "#1A1A2E", muted: "#EDE8DC", mutedFg: "#7A6E5F", border: "#D4CCBA",
    accent: "#C0392B", pavilion: "#1A3520", pavilionFg: "#FFFDF8",
    pavilionMuted: "rgba(255,255,255,0.55)",
  };
  return <PalettePreview p={p} name="Cricket Green" />;
}
  ...
  return (
    <div style={{ width: 390, minHeight: 780, background: p.bg, fontFamily: "system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, padding: "16px 16px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {[
          { opp: "vs Northside CC", score: "72 (45)", result: "Won", wickets: "2/28" },
          { opp: "vs Valley XI", score: "18 (22)", result: "Lost", wickets: "1/34" },
          { opp: "vs Riverside SC", score: "94 (78)", result: "Won", wickets: "3/21" },
        ].map((m, i) => (
          <div key={i} style={{ background: p.card, border: `1px solid ${p.border}`, borderRadius: 14, padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ background: m.result === "Won" ? p.primary : p.accent, color: "#fff", borderRadius: 8, padding: "4px 10px", fontSize: 12, fontWeight: 700 }}>
              {m.result}
            </div>
          </div>
        ))}
